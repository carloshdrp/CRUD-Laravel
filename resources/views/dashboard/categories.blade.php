...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Gerencie as categorias - Total: '.count($categories) ) }}
        </h2>
    </x-slot>
                    ...
                    @foreach($categories as $category)
                        <tr class="hover:bg-gray-50">
                            ...
                            <th class="flex gap-3 px-6 py-4 font-normal text-neutral-800">
                                <div class="font-medium text-lg text-gray-700">{{ $category->name }}</div>
                            </th>

                            <td class="px-6 py-4">{{ ($category->created_at)->format('d/m/Y') }}</td>

                            <td class="px-6 py-4">{{ ($category->updated_at)->format('d/m/Y - H:i:s') }}</td>

                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-4">
                                    <a x-data="{ tooltip: 'Delete' }" href="categories/delete/{{ $category->id }}">
                                        <svg
                                    ...
                                    <a x-data="{ tooltip: 'Edite' }" href="categories/edit/{{ $category->id }}">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            fill="none"
                                            viewBox="0 0 24 24"
                            ...
                            <td colspan="5" class="px-6 py-4">
                                <div class="flex items-center justify-center gap-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                         stroke-width="1.5" stroke="currentColor" class="w-6 h-6 2xl:w-8 2xl:h-8">
